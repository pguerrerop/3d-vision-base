...
class DuplicateRunIdError(ValueError):
    """Raised when a run_id already exists in process_run and the caller asked
    not to silently overwrite it (see write_run's strict=True)."""


def write_run(conn, entry: dict[str, Any], *, strict: bool = False) -> None:
    """Insert one run row. Caller owns the transaction.

    strict=False (default): INSERT OR REPLACE, used by the one-shot legacy
    migration (import_entries), which must tolerate a stale runs.json that
    duplicated or clobbered entries under the old JSON-rewrite scheme.

    strict=True: a real INSERT, used by the live append path (append_run).
    run_id is a global primary key here, generated by callers (uuid4-based --
    see processes/service.py:_new_id); a collision is never expected and
    almost certainly means an id generator regressed, so this raises instead
    of silently overwriting another run's row.
    """
    ...
    verb = "INSERT" if strict else "INSERT OR REPLACE"
    try:
        conn.execute(
            f"{verb} INTO process_run({', '.join(RUN_COLUMNS)}, summary_json)"
        ...
        raise DuplicateRunIdError(
            f"run_id {run_id!r} already exists in process_run; refusing to overwrite it"
        ) from exc
...
def append_run(data_dir: Path, entry: dict[str, Any]) -> None:
    ...
    with db.transaction(conn):
        write_run(conn, entry, strict=True)
