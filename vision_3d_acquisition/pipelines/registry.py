...
PIPELINES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "stages": [
            ...
            _stage(stage_id="detect_belt_plane", display_name="Detect belt plane", description="Detect/fits the conveyor background plane and emits inlier/residual diagnostics.", required_modalities=["heightmap"], optional_modalities=["reflectance", "rgb"], produced_artifact_kinds=["image", "json"], object_outputs=False, supports_real_time=True, dependencies=["input"]),
            _stage(stage_id="normalize_heights_to_plane", display_name="Normalize heights to belt plane", description="Recompute height_above_belt_mm using detected plane and emit near-zero background QA metrics.", required_modalities=["heightmap"], optional_modalities=["reflectance", "rgb"], produced_artifact_kinds=["image", "json"], object_outputs=False, supports_real_time=True, dependencies=["detect_belt_plane"]),
            _stage(stage_id="remove_belt_segment_objects", display_name="Remove belt + segment objects", description="Remove belt/background using normalized heights and isolate object components.", required_modalities=["heightmap"], optional_modalities=["reflectance", "rgb"], produced_artifact_kinds=["image", "overlay", "json"], object_outputs=True, supports_real_time=True, dependencies=["normalize_heights_to_plane"]),
        ...
        "composition": {
            ...
            "artifact_flow": {
                ...
                "normalize_heights_to_plane": ["normalized_heightmap", "normalized_height_histogram", "normalization_debug"],
                "remove_belt_segment_objects": ["normalized_height_threshold_mask", "cleaned_object_mask", "connected_components_overlay", "segmentation_debug"],
